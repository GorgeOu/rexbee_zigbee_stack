...
#define	ZCL_FRAME_CONTROL_VIA_BINDING		  BIT(7)
...
typedef uint8_t app_reporting_plugin_memory_entry_t[32];
...
typedef bool (*app_decode_zcl_cmd_handler_t)(uint8_t trigger_type,
...
#define APP_DECODE_ZCL_CMD_SIG_MAX_SIZE	16

//e.g. {0x0104, 0x0006, 0x0000, 0x01, 0x00, "", onoff_on_command_handler, {true, true, false, true}, {sig_offset}}
typedef struct _app_decode_zcl_cmd_list
{
	uint16_t profile;
	uint16_t cluster;
	uint16_t mfg_code;
	...
	uint8_t  command_id;
	const char* format;
	app_decode_zcl_cmd_handler_t handler;
	struct
	{
	...
	const uint8_t sig_offset[APP_DECODE_ZCL_CMD_SIG_MAX_SIZE];
} app_decode_zcl_cmd_list_t;

typedef bool (*app_check_attr_legality_t)(uint8_t  trigger_type,
										  uint8_t  endpoint,
										  uint8_t  data_type,
...
typedef void (*app_attr_handler_t)(uint8_t  trigger_type,
								   uint8_t  endpoint,
								   uint8_t  data_type,
								   uint8_t  data_length,
								   uint8_t* data);

typedef struct app_attr_info_s
{
	uint8_t	 endpoint;
	uint8_t	 mask;
	uint16_t cluster_id;
...
typedef struct app_attr_desc_s
{
	app_attr_info_t			 	app_attr_info;
	app_check_attr_legality_t	app_check_attr_legality;
	app_attr_handler_t 		 	app_attr_handler;
} app_attr_desc_t;
