...
#define GET_PWMID(gpio, func)     ((gpio==GPIO_PA0) ? 0 : (  \
                     ...
                     (gpio==GPIO_PD3) ? 1 : (  \
                     (gpio==GPIO_PD4) ? 2 : (  \
                     (gpio==GPIO_PD5) ? 0 : 0xFF  \
                    ))))))))))))))))))))))

#define GET_AS_PWM(gpio, func)     ((gpio==GPIO_PA0) ? 26 : (  \
									 (gpio==GPIO_PA2) ? 20 : (  \
									 (gpio==GPIO_PA3) ? 21 : (  \
									 (gpio==GPIO_PA4) ? 22 : (  \
									 (gpio==GPIO_PB0) ? 23 : (  \
									 ...
									 (gpio==GPIO_PB5) ? 25 : (  \
									 (gpio==GPIO_PC0) ? 30 : (  \
									 (gpio==GPIO_PC1) ? ((func==AS_PWM_SECOND) ? 27 : 20) : (  \
									 (gpio==GPIO_PC2) ? 20 : (  \
									 (gpio==GPIO_PC3) ? 21 : (  \
									 (gpio==GPIO_PC4) ? ((func==AS_PWM_SECOND) ? 26 : 22) : (  \
									 (gpio==GPIO_PC5) ? 29 : (  \
									 (gpio==GPIO_PC6) ? 30 : (  \
									 (gpio==GPIO_PC7) ? 31 : (  \
									 (gpio==GPIO_PD2) ? 23 : (  \
									 (gpio==GPIO_PD3) ? 27 : (  \
									 (gpio==GPIO_PD4) ? 28 : (  \
									 (gpio==GPIO_PD5) ? ((func==AS_PWM_SECOND) ? 20 : 27) : 0xff\
									))))))))))))))))))))))

#define PORT_PIN_CONVERT(portPin) (GPIO_PinTypeDef)((uint16_t)((uint16_t)((uint16_t)portPin & 0x00F0) << 4) | (uint16_t)(1 << (uint16_t)((uint16_t)portPin & 0x000F)))
...
void gpio_clear_global_gpio_func(GPIO_PinTypeDef pin);
/**
...
int gpio_is_input_en_fix(GPIO_PinTypeDef pin);

/**
 * @brief      This function determines whether the output function of a pin is enabled.
 * @param[in]  pin - the pin needs to determine whether its output function is enabled.
 * @return     1: the pin's out state high level
 *             0: the pin's out state low level
 */
static inline int gpio_get_out_state(GPIO_PinTypeDef pin)
{
...
GPIO_PullTypeDef gpio_get_up_down_resistor(GPIO_PinTypeDef gpio);
