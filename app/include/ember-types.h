...
/* first, a generic two token concatenator */
#define CT_CONCAT(a, b)      a##b

/* helper that injects the line number */
#define CT_ASSERT_LINE(expr, name, line) \
    typedef char CT_CONCAT(assertion_##name##_, line)[ (expr) ? 1 : -1 ]

/* the public macro: only take expr & name, line is filled in via __LINE__ */
#define CT_ASSERT(expr, name) \
    CT_ASSERT_LINE((expr), name, __LINE__)

#define	REXENSE_MANUFACTURER_CODE				0x1163

#define ZIGBEE_2_4_GHZ_ALL_802_15_4_CHANNELS_MASK 0x07FFF800UL

#define HA_PROFILE_ID   (0x0104)
#define ZA_MAX_HOPS 	12

/** zll initiator comm state
 *
 */
typedef enum _zll_initiator_comm_state
{
	ZLL_INITIATOR_COMM_STATE_NOT_FOUND,
	ZLL_INITIATOR_COMM_STATE_LINK_NO_RESP,
	ZLL_INITIATOR_COMM_STATE_LINK_NEW_TARGET,
	ZLL_INITIATOR_COMM_STATE_LINK_UPDATE,
	ZLL_INITIATOR_COMM_STATE_TOUCH_RESET,
	ZLL_INITIATOR_COMM_STATE_REJOIN_FAILURE,
} zll_initiator_comm_state_t;

/** plugin struct definition
 *
 */
typedef struct _window_convering_calback
{
	/*typedef EmberAfStatus (*open_callback)(uint8_t endpoint)*/
	void* open_callback;
	/*typedef EmberAfStatus (*close_callback)(uint8_t endpoint)*/
	void* close_callback;
	/*typedef EmberAfStatus (*stop_callback)(uint8_t endpoint)*/
	void* stop_callback;
	/*typedef EmberAfStatus (*lift_value_callback)(uint8_t endpoint, uint16_t value)*/
	void* lift_value_callback;
	/*typedef EmberAfStatus (*lift_percent_callback)(uint8_t endpoint, uint8_t percent)*/
	void* lift_percent_callback;
	/*typedef EmberAfStatus (*tift_value_callback)(uint8_t endpoint, uint16_t value)*/
	void* tift_value_callback;
	/*typedef EmberAfStatus (*tift_percent_callback)(uint8_t endpoint, uint8_t percent)*/
	void* tift_percent_callback;
} window_convering_calback_t;

typedef enum _window_convering_action_enum
{
	WINDOEW_CONVERING_OPEN = 0x00,
	WINDOEW_CONVERING_CLOSE,
	WINDOEW_CONVERING_STOP,
	WINDOEW_CONVERING_LIFT_VALUE,
	WINDOEW_CONVERING_LIFT_PERCENT,
	WINDOEW_CONVERING_TILT_VALUE,
	WINDOEW_CONVERING_TILT_PERCENT,
} window_convering_action_enum_t;

typedef struct _app_scene_plugin_scene_entry
{
	uint16_t cluster;
	uint16_t attribute;
	uint8_t  data_type;
	uint8_t  data_max_length;
	uint8_t  reserved[2];
} app_scene_plugin_scene_entry_t;

#define APP_SCENE_TABLE_MAX_DATA_SIZE	80

typedef struct _app_scene_plugin_scene_table_entry
{
  ...
  uint8_t   size;
  uint32_t  use_member_mask;
  uint32_t  reserved;
  uint8_t   data[APP_SCENE_TABLE_MAX_DATA_SIZE];
} app_scene_plugin_scene_table_entry_t;

typedef uint8_t StackZigbeeMessageType;
enum
{
	ZIGBEE_MESSAGE_TYPE_UNKNOW,
	ZIGBEE_MESSAGE_TYPE_MAC,
	ZIGBEE_MESSAGE_TYPE_NWK,
	ZIGBEE_MESSAGE_TYPE_APS,
	ZIGBEE_MESSAGE_TYPE_APS_COMMAND,
	ZIGBEE_MESSAGE_TYPE_APS_INTERPAN,
	ZIGBEE_MESSAGE_TYPE_ZDO,
	ZIGBEE_MESSAGE_TYPE_ZCL,
};

/** frameworkRegisterCallback for app register require callback yuxiaoqiang add*/
typedef int (*frameworkRegisterCallback)(void* arg);
typedef int (*eventCallback)(void);

typedef struct
{
	frameworkRegisterCallback powerManagementEnterSleepCallback;
} PhyLayerRegisterCallback;

typedef struct
{
	/*typedef void (*PreHalConfig)(void)*/
	void* appPreHalConfig;
	/*typedef void (*GetTokenToRam)(void)*/
	void* appGetTokenToRam;
	/*typedef void (*FreshNetworkStart)(void)*/
	void* appFreshNetworkStart;
	/*typedef void (*LeaveNetworkDetect)(void)*/
	void* appLeaveNetworkDetect;
	/*typedef void (*stackStatusChaned)(EmberStatus status,
									    EmberNetworkStatus last_network_state,
									    EmberNetworkStatus network_state)*/
	void* appStackStatusChanged;
	/*typedef void (*ResetIndicate)(void)*/
	void* appResetIndicate;
	/*typedef void (*CertCheckFailedInit)(void)*/
	void* appCertCheckFailedInit;
	/*typedef void (*CertCheckFailedIndicate)(void)*/
	void* appCertCheckFailedIndicate;
	/*typedef void (*MessageSentCallback)(EmberStatus status,
										  StackZigbeeMessageType type,
										  uint8_t buffer,
										  EmberApsFrame* apsFrame,
										  uint16_t nextHopsNodeId,
										  uint16_t destination,
										  void* data)*/
	void* appMessageSentCallback;
	/*
	typedef void (*ReceviedPacketHandler)(StackZigbeeMessageType type,
										  uint8_t  buffer,
										  uint8_t  payloadIndex,
										  uint8_t* payload,
										  uint8_t  payloadLength,
										  void *data)*/
	void* appReceviedPacketHandler;
} RexUniversalLibRegisterCallback;

extern RexUniversalLibRegisterCallback rexUniversalLibCallbacksRegister;

typedef struct _eeprom_default_data
{
	uint16_t id;
	uint8_t length;
	uint8_t member;
	const uint8_t* data;
} eeprom_default_data_t;

typedef struct _recovery_network_config
{
	uint8_t  recovery_attempt_section_min;
	uint8_t  recovery_attempt_section_max;
	bool  	 recovery_secure_enable;
	uint32_t channel_mask;
	uint32_t recovery_gap_ms;
} recovery_network_config_t;

typedef bool (*stack_application_zcl_handle_t)(void);

typedef void (*ota_start_notify_t)(void);
typedef void (*ota_in_progress_indicate_t)(uint8_t received_percentage);
typedef void (*ota_recevied_finished_notify_t)(bool firmware_check);
typedef void (*ota_reboot_upgrade_firmware_notifiy_t)(void);
typedef void (*ota_single_request_timeout_t)(uint32_t offset, uint32_t image_length);
typedef void (*ota_upgrade_abort_t)(void);
typedef void (*ota_upgrade_wait_for_data_t)(void);
typedef void (*external_mcu_ota_notify_t)(uint16_t image_type);
typedef void (*external_mcu_next_image_response_t)(uint16_t mfg_code, uint16_t image_type, uint32_t version, uint32_t image_length);
typedef void (*external_mcu_image_block_response_t)(uint8_t status, uint16_t mfg_code, uint16_t image_type, uint32_t version, uint32_t offset, uint8_t length, uint8_t* data);
typedef void (*ota_hal_flash_read_t)(uint32_t address, uint8_t* data, uint16_t length);
typedef void (*ota_hal_flash_write_t)(uint32_t address, uint8_t* data, uint16_t length);
typedef void (*ota_hal_flash_erase_t)(uint32_t address);

typedef struct _app_ota_plugin_handle_list
{
	ota_start_notify_t 						ota_start_notify;
	ota_in_progress_indicate_t 				ota_in_progress_indicate;
	ota_recevied_finished_notify_t 			ota_recevied_finished_notify;
	ota_reboot_upgrade_firmware_notifiy_t 	ota_reboot_upgrade_firmware_notifiy;
	ota_single_request_timeout_t 			ota_single_request_timeout;
	ota_upgrade_abort_t 					ota_upgrade_abort;
	ota_upgrade_wait_for_data_t 			ota_upgrade_wait_for_data;
	external_mcu_ota_notify_t 				external_mcu_ota_notify;
	external_mcu_next_image_response_t  	external_mcu_next_image_response;
	external_mcu_image_block_response_t 	external_mcu_image_block_response;

	ota_hal_flash_read_t  ota_hal_flash_read;
	ota_hal_flash_write_t ota_hal_flash_write;
	ota_hal_flash_erase_t ota_hal_flash_erase;
} app_ota_plugin_handle_list_t;

typedef struct _zcl_identify_command_list
{
	bool (*identify_callback)(uint8_t, uint16_t);
	bool (*identify_query_callback)(uint8_t);
	bool (*identify_trigger_effect_callback)(uint8_t, uint8_t, uint8_t);
	bool (*identify_query_response_callback)(uint8_t, uint16_t);
} zcl_identify_command_list_t;

typedef zcl_identify_command_list_t app_identify_plugin_handle_list_t;

typedef enum _user_zigbee_type_config_enum
{
	ZIGBEE_TYPE_ZIGBEE_HA_1_2_CONFIG  		= 0,
	ZIGBEE_TYPE_ZIGBEE_3_0_CONFIG 	  		= 1,
	ZIGBEE_TYPE_ZIGBEE_INSTALL_CODE_CONFIG  = 2,
} user_zigbee_type_config_enum_t;

typedef struct _app_user_device_info
{
	char const* firmware_print_version;
	char const* model_id;
	char const* manufacture_name;
	uint16_t 	manufacture_code;
	//pa module support[-4,-1,6,10,14,15,16,17,18,19,20],
	//non-pa module support[-10,-9,...,9,10]
	int8_t 		radio_tx_power;
	user_zigbee_type_config_enum_t zigbee_type;
	bool		default_zcl_message_aps_ack_enable;
	bool   		send_device_announce_after_reboot;
} app_user_device_info_t;

typedef struct
{
	uint8_t  endpoint;
	uint16_t zone_type;
} zcl_zone_type_list_t;

typedef bool (*app_custom_zcl_cmd_callback_t)(uint8_t trigger_type, uint8_t endpoint, uint8_t* data, uint16_t length, uint8_t* default_resp_status);

//e.g. {0x0104, 0x0006, 0x0000, 0x01, 0x00, onoff_on_command_handler, {true, true, false, true}}
typedef struct _app_custom_zcl_cmd_handle
{
	uint16_t profile;
	uint16_t cluster;
	uint16_t mfg_code;
	uint8_t  endpoint;
	uint8_t  command_id;
	app_custom_zcl_cmd_callback_t handle;
	struct
	{
		uint8_t zcl_server_to_client_command 	: 1;
		uint8_t auto_default_resp  				: 1;
		uint8_t zcl_mfg_command    				: 1;
		uint8_t zcl_spec_command   				: 1;
		uint8_t reserved						: 4;
	};
} app_custom_zcl_cmd_handle_t;

#define OTA_PLUGIN_USE_BCD_DATE_CODE_AS_OTA_IMAGE_VERSION 	0xffffffffUL

#define REX_ZCL_STATUS_SUCCESS		0x00
#define REX_ZCL_STATUS_FAILURE		0x01


#define REX_STACK_STATUS_SUCCESS	0x00
#define REX_STACK_STATUS_FAILURE	0x01
//#define FAST_JOIN_MAX_SUPPORT_EP_NUMBER	6
//
//typedef	struct _fast_join_endpoint_info
//{
//	uint16_t	device_id;
//	uint16_t	rex_od;
//	uint8_t		endpoint;
...
//			uint8_t	light	: 4;
//			uint8_t	meter	: 2;
//		} _rex_od_detail_device_type_t;
//
//	} _union_device_type_t;
//
//} fast_join_endpoint_info_t;
//
//typedef	struct _fast_join_device_info
//{
//	uint8_t		mac_address[8];		// in Little endian
//	uint16_t	rex_od;
//
//	uint8_t		basic_zcl_version;
//	uint8_t		basic_hw_version;
//	uint8_t		basic_model_id[6];
//	uint8_t		basic_bcd_date_code[4];
//
//	struct
//	{
//		uint8_t	power_source 	: 3;	// unknow(0) AC(1) Bat(3) DC(4)
//		uint8_t	profile		 	: 2;	// ZHA(0) ZLL(1)
//		uint8_t	spec_ep_config	: 1;	// 1 : Device has multiple EPs, such as 16 EPs Thermostat, other 0
//		uint8_t	reserved		: 2;
//	} info;
//
//	uint8_t ep_number;	// 1 ~ 6
//	fast_join_endpoint_info_t ep_info[FAST_JOIN_MAX_SUPPORT_EP_NUMBER];
//
//} fast_join_device_info_t;

typedef struct
{
	bool (*on_read_attr)(uint8_t endpoint,
						 uint16_t cluster,
						 uint16_t attr,
						 uint8_t mask,
						 uint16_t mfg_code,
						 ...
						 uint16_t max_length);
	bool (*on_write_attr)(uint8_t endpoint,
							uint16_t cluster,
							uint16_t attribute,
							uint8_t mask,
							uint16_t mfg_code,
	...
	void* on_report_attr;
	void* on_write_attr_resp;
	void* on_read_attr_resp;
	void* on_default_resp;
} zcl_internal_global_hooks_t;
...
extern EmberEUI64 stkDeviceEui64;
#define emberGetEui64() (stkDeviceEui64)
...
#endif
{
  ...
  /** The node is joined to a network, but trust center is not responding*/
  EMBER_JOINED_NETWORK_NO_TC,
};
